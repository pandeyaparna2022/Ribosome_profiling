...
mkdir ${destination}/trimmed_data

cd ${destination}/trimmed_data
...
do cutadapt -j 4 -q 25 -a CTGTAGGCACCATCAAT -m 25 --discard-untrimmed -o $(basename ${i} .fastq.gz)_clpd.fastq.gz \
...
do cutadapt -j 4 -q 25 --cut -4 -m 25 -o $(basename ${i} .fastq.gz)_tr.fastq.gz ${i} 1> $(basename ${i} .fastq.gz)_tr_log.txt; done

#QC analysis of the processed reads

mkdir ${destination}/QC_trimmed_reads
cd ${destination}/QC_trimmed_reads

for j in ${destination}/trimmed_data/*_tr.fastq.gz; do ln -s "$j" . ; done
