...
for x in ${source}/*tr.fastq.gz; do gunzip ${x}; done
...
for x in ${destination}/filtered_data/*no_r-t-sno-sn-RNA.fastq; \
do bowtie -t -p 4 -v 1 -m 1 --best --strata --norc ${indices}/GRCh38_p13_APPRIS_CDS_plus18 -q ${x} -S $(basename ${x} .fastq)_GRCh38_p13_APPRIS_CDS.sam 2> $(basename ${x} .fastq)_GRCh38_p13_APPRIS_CDS_log.txt; done 
